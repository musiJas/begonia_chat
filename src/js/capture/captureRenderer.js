...
const {
    ipcRenderer, clipboard, nativeImage, remote } = require('electron')

const fs = require('fs')
const { getScreenSources } = require('./desktopCapture')
const { CaptureEditor } = require('./captureEditor')
...
getScreenSources({}, (imgSrc) => {
    ...
    $btnSave.addEventListener('click', () => {
        ...
        let url = capture.getImageUrl();
        remote.getCurrentWindow().hide();
        ipcRenderer.on('save-selected-directory', (event, path) => {
            console.log(path);
            console.log(event);
            if(path){

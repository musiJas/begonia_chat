...
const { getCurrentScreen,changeLuminance} = require('./captureUtil')
...
class CaptureEditor extends Event {
    ...
    onMouseDrag(e) {
        ...
        if (!this.startPoint.moved) {
            if (Math.abs(this.startPoint.x - pageX) > 15 || Math.abs(this.startPoint.y - pageY) > 15) {
    ...
    drawRect() {
        ...
        if (w && h) {
            ...
            this.ctx.putImageData(changeLuminance(imageData,0.2), margin * scaleFactor, margin * scaleFactor)
        }
        this.ctx.fillStyle = '#ffffff'
        this.ctx.strokeStyle = '#03a9f4'
        //this.ctx.lineWidth = 2 * this.scaleFactor
        this.ctx.lineWidth =  this.scaleFactor
    ...
    getImageUrl() {
        ...
        if (w && h) {
            ...
            canvas.height = h
            let ctx = canvas.getContext('2d')
            ctx.putImageData(changeLuminance(imageData,0.2), 0, 0)

...
class httpClient {
   ...
   * @returns {Promise<unknown>}
   */
  static async staticFileFetch(url = '', options = {}): Promise {
    const defaultOptions = {
      /*允许携带cookies*/
      credentials: 'include',
      /*允许跨域**/
   ...
   * @returns {Promise<unknown>}
   */
  postFile(url, paramsArgs = {}, option = {}) {
    const options = Object.assign({ method: 'POST' }, option)
    //一般我们常用场景用的是json，所以需要在headers加Content-Type类型

    // for(let key of Object.keys(paramsArgs)){
    // }
    options.body =paramsArgs
    return httpClient.staticFileFetch(url, options) //类的静态方法只能通过类本身调用
  }
...
export const { post, get, put,postFile } = requestFun

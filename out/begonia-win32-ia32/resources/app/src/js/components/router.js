...
import type {ConnectionT, RouteT, IrcState, Dispatch } from '../common/constants'
import LoginCreator from  './loginCreator'
import MainPage from  './mainPage'
import GlobalStyle from './globalStyle'
import { LOGIN_SCCUESS } from '../common/actions'
...
class  RouteComponent extends React.PureComponent<props>{
  ...
  renderContents() {
    ...
    sessionStorage.setItem("server", route.credential.server);
    sessionStorage.setItem("port", route.credential.port);
    //route.type
    switch (route.type) {  
      case 'LOGIN_SCCUESS':
        return <MainPage credential={route.credential}/>
      case 'LOGIN_TODO':
        return <LoginCreator  />
      default:
        return <LoginCreator />
...
export default connect(
    (state: LoginState, ownProps): $Shape<Props> => {
    return {
